...
window.showMasterDashboard = async () => {
  ...
  try {
    ...
    if (allMatchings && allMatchings.length > 0) {
      ...
      console.log('매칭 정보가 없어 자동으로 새로고침 시도');
...
window.loadMasterDashboardData = async () => {
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      const totalOrgs = mainOrgs.length + subOrgs.length;
      
      document.getElementById('total-orgs-count').textContent = '55';
      ...
      console.log('기관 목록 로드 완료:', totalOrgs);
    } else {
      ...
      try {
        ...
        if (savedSchedules) {
          schedules = JSON.parse(savedSchedules);
          console.log(`[DEBUG] API 실패 시 로컬 스토리지에서 ${schedules.length}개의 일정 가져옴`);
        }
      } catch (e) {
        console.error('로컬 스토리지에서 일정 데이터 가져오기 실패:', e);
      }
      ...
      console.log(`[DEBUG] API 실패 시 완료율 계산: ${completedSchedules}/${totalSchedules} = ${completionRate}%`);
    }
    ...
    try {
      console.log('매칭 정보 처리 시작');
      
      // Google Sheets API에서 매칭 데이터 가져오기
      const committeeMatchingsResponse = await fetch('/api/committees/matching');
      
      if (!committeeMatchingsResponse.ok) {
        throw new Error(`매칭 정보 API 응답 오류: ${committeeMatchingsResponse.status}`);
      }
      
      const committeeMatchingsData = await committeeMatchingsResponse.json();
      console.log('Google Sheets API 응답 수신:', committeeMatchingsData);
      
      if (committeeMatchingsData && Array.isArray(committeeMatchingsData)) {
        // API에서 가져온 데이터 사용
        allMatchings = committeeMatchingsData;
        ...
        // 샘플 매칭 데이터 생성
        allMatchings = [
          { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48170002', orgName: '진주노인통합지원센터', region: '경상남도', role: '주담당' },
          { committeeId: 'C002', committeeName: '문일지', orgCode: 'A48120002', orgName: '창원도우누리노인종합재가센터', region: '경상남도', role: '주담당' },
          { committeeId: 'C003', committeeName: '김수연', orgCode: 'A48310001', orgName: '거제노인통합지원센터', region: '경상남도', role: '주담당' },
          { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48310001', orgName: '거제노인통합지원센터', region: '경상남도', role: '부담당' }
        ];
        console.log('샘플 매칭 데이터 생성:', allMatchings.length);
      }
    } catch (error) {
      console.error('매칭 정보 처리 중 오류:', error);
      
      // 오류 발생 시 샘플 매칭 데이터 생성
      allMatchings = [
        { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48170002', orgName: '진주노인통합지원센터', region: '경상남도', role: '주담당' },
        { committeeId: 'C002', committeeName: '문일지', orgCode: 'A48120002', orgName: '창원도우누리노인종합재가센터', region: '경상남도', role: '주담당' },
        { committeeId: 'C003', committeeName: '김수연', orgCode: 'A48310001', orgName: '거제노인통합지원센터', region: '경상남도', role: '주담당' },
      ...
      console.log('오류 발생 시 샘플 매칭 데이터 생성:', allMatchings.length);
    }
...
window.refreshMatchingData = async () => {
  ...
  // 기본 매칭 데이터 설정 (API 호출 실패 시 사용)
  if (!window.defaultMatchings) {
    window.defaultMatchings = [
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48170002', orgName: '산청한일노인통합복지센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48820003', orgName: '함안노인복지센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48170003', orgName: '진주노인통합지원센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48240001', orgName: '김해시니어클럽', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48240002', orgName: '창원도우누리노인종합재가센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48840001', orgName: '마산시니어클럽', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48840002', orgName: '거제노인통합지원센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48850001', orgName: '동진노인종합복지센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48850002', orgName: '생명의전화노인복지센터', role: 'main' },
      { committeeId: 'C001', committeeName: '신용기', orgCode: 'A48170001', orgName: '보현행정노인복지센터', role: 'main' }
    ];
  }
  ...
  try {
    ...
    const existingMatchings = allMatchings.length > 0 ? [...allMatchings] : (window.defaultMatchings || []); // 기존 데이터 복사본 사용
    ...
    if (!matchingData.data || !matchingData.data.matchings || !Array.isArray(matchingData.data.matchings)) {
      ...
      matchingData.data = { matchings: existingMatchings.length > 0 ? existingMatchings : window.defaultMatchings || [] };
    }
    
    // 성공하든 실패하든 여기서 데이터 처리
    const rawMatchings = matchingData.data.matchings || [];
    
    // 매칭 데이터가 비어있으면 기본 데이터 사용
    if (rawMatchings.length === 0 && window.defaultMatchings && window.defaultMatchings.length > 0) {
      console.log('매칭 데이터가 비어있어 기본 데이터를 사용합니다.');
      matchingData.data.matchings = window.defaultMatchings;
    }
...
window.updateMatchingTable = () => {
  ...
  // 기관 데이터가 없는 경우 매칭 데이터에서 추출 시도
  if (!allOrganizations || allOrganizations.length === 0) {
    console.log('기관 데이터가 없습니다. 매칭 데이터에서 추출 시도...');
    ...
    if (allMatchings && allMatchings.length > 0) {
      ...
      allMatchings.forEach((match, index) => {
        ...
        if (orgCode) {
          ...
          // 처음 10개의 기관만 로그로 출력
          if (index < 10) {
            console.log(`기관 정보 추출 [${index}]: ${orgCode} - ${orgName} (${region})`);
          }
        }
      });
    ...
    // 여전히 기관 데이터가 없는 경우 샘플 데이터 생성
    if (!allOrganizations || allOrganizations.length === 0) {
      console.log('기관 데이터 추출 실패, 샘플 데이터 생성');
      
      // 샘플 기관 데이터 생성
      allOrganizations = [
        { code: 'A48170002', name: '진주노인통합지원센터', region: '경상남도' },
        { code: 'A48120002', name: '창원도우누리노인종합재가센터', region: '경상남도' },
        { code: 'A48310001', name: '거제노인통합지원센터', region: '경상남도' }
      ];
    }
  }
  ...
  const filteredOrgs = allOrganizations.filter(org => {
    ...
    // 기관 코드 확인
    if (!orgCodeVal) {
      console.log('기관 코드가 없는 기관 발견:', org);
      // 기관 코드가 없는 경우에도 표시
      return true;
    }
  ...
  Object.keys(regionGroups).sort().forEach(region => {
    ...
    regionGroups[region].sort((a, b) => {
      ...
      console.log(`기관 ${orgCodeForMatching} 매칭 정보:`, { 주담당: mainMatchings.length, 부담당: subMatchings.length });
