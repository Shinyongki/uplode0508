...
window.loadIndicatorsByPeriod = async (period) => {
  ...
  try {
    ...
    if (!selectedOrganization) {
      ...
      if (typeof window.showMessage === 'function') {
        window.showMessage('기관을 먼저 선택해주세요.', 'error');
      } else {
        alert('기관을 먼저 선택해주세요.');
      }
    ...
    // 주기 별 지표 API 호출
    console.log(`주기별 지표 요청 시작: ${period}, 기관 코드: ${orgCode}`);
    ...
    try {
      ...
      console.log(`지표 응답 수신: 상태 ${data.status}, ${data.data?.indicators?.length || 0}개 항목`);
